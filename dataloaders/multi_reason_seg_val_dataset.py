...
class MultiReasonSegValDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        multi_reason_seg_data, split = val_dataset.split("|")        
        # Define splits to load: 'test' merges test_less and test_many
        splits_to_load = ['test_less', 'test_many'] if split == 'test' else [split]
        reason_file = []
        for sub_split in splits_to_load:
            json_file_name = os.path.join(base_image_dir, 'multi_reason_seg', multi_reason_seg_data, f'MUSE_{sub_split}.json')
            with open(json_file_name, 'r') as f:
                sub_data = json.load(f)
                reason_file.extend(sub_data)
       
        self.multi_reason_seg_data = reason_file[20:80]
        print("number of multi_reason_seg samples(val split): ", len(reason_file))

       
    def __len__(self):
        return len(self.multi_reason_seg_data)
