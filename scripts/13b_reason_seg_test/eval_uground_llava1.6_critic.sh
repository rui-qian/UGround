...
deepspeed --include "localhost:7" --master_port="$MASTER_PORT" train_ds.py \
  --model_key="$MODEL_KEY" \
  --version="./runs/UGround-13b_reason_seg_test_llava1.6_critic/hf-UGround-13b_reason_seg_test_llava1.6_critic" \
  --dataset_dir='../dataset_sesame' \
  --dataset="vqa" \
  --sample_rates="1" \
  --exp_name="UGround-13b_reason_seg_test_llava1.6_critic/eval" \
