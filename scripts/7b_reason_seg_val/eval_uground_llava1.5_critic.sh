...
deepspeed --include "localhost:6" --master_port="$MASTER_PORT" train_ds.py \
  --model_key="$MODEL_KEY" \
  --version="./runs/UGround-7b_reason_seg_val_llava1.5_critic/hf-UGround-7b_reason_seg_val_llava1.5_critic" \
  --dataset_dir='../dataset_sesame' \
  --dataset="vqa" \
  --sample_rates="1" \
  --exp_name="UGround-7b_reason_seg_val_llava1.5_critic/eval" \
