...
def encode_text_with_prompt_ensemble(model, texts, device, prompt_templates=None):
    ...
    for t in texts:
        ...
        # (Pdb) p class_embeddings.shape
        # torch.Size([85, 512])
        class_embeddings = model.encode_text(prompted_t)
        # (Pdb) class_embeddings.norm(dim=-1, keepdim=True).shape
        # torch.Size([85, 1])
        
        class_embeddings /= class_embeddings.norm(dim=-1, keepdim=True)
        # (Pdb) class_embeddings.mean(dim=0).shape
        # torch.Size([512])
...
def clip_feature_surgery(image_features, text_features, redundant_feats=None, t=2):
    ...
    if redundant_feats != None:
        ...
        # (Pdb) image_features.reshape(b, n_i, 1, c).shape
        # torch.Size([1, 197, 1, 512])
        # (Pdb) text_features.reshape(1, 1, n_t, c).shape
        # torch.Size([1, 1, 59, 512])
        # (Pdb) feats *= w.reshape(1, 1, n_t, 1)
        # (Pdb) p feats.shape
        # torch.Size([1, 197, 59, 512])
